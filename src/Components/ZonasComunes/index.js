...
const ZonasComunes = ({ conjunto, user, currentVivienda }) => {
  ...
  const getStringDataLocation = () => {
    if (user?.tipoUsuario === 'Residente') {
      return `${currentVivienda.idconjunto}/${user.id}/${currentVivienda.idunidaddevivienda}`;
    }
    return `${conjunto.idconjunto}/${conjunto.idusuarioadministrador}/${conjunto.id}`;
  };
  ...
  return (
    <Box sx={{ flexGrow: 1, mx: 0 }} className="card">
      ...
      <Typography align="center" component="p" gutterBottom>
        Siempre divertirse y compartir va ser muy importante para crear una comunidad mas
        unida es por eso que aqui podrás añadir las zonas comunes con que cuenta tu conjunto
        con el fin de que estas puedan ser alquiladas y usadas por otros usuarios
      </Typography>

      <Grid container justifyContent="center" alignItems="flex-start" className="formConjuntos">
        <Grid item xs={4}>
          <Paper>
            <img alt="" src="/sport1.png" height="270px" width="270px" />
          </Paper>
        </Grid>
        <Grid item xs={4}>
          <Paper>
            <img alt="" src="/sport2.png" height="270px" width="270px" />
          </Paper>
        </Grid>
        <Grid item xs={4}>
          <Paper>
            <img alt="" src="/sport3.png" height="270px" width="270px" />
          </Paper>
        </Grid>
      </Grid>
      ...
      <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }} className="card">
        <Grid container spacing={2} justifyContent="center" alignItems="flex-start">
          <Grid item xs={4}>
            <DropForm
              ...
              location="admin"
              enableSubmit={false}
              submited={handleOnChange}
              isenable={true}
            />
          </Grid>
          <Grid item xs={4}>
            <TextField required id="tiempoAlquiler" name="tiempoAlquiler" label="tiempo Alquiler/h" variant="outlined" type="number" />
          </Grid>
          <Grid item xs={4}>
            <TextField
              required
        ...
        <FormControl component="fieldset">
          ...
          <RadioGroup row aria-label="disponible" name="row-radio-buttons-group">
        ...
        <Grid container spacing={2} justifyContent="center" alignItems="flex-start">
          <Grid item xs={6}>
            <TextField required id="tiempoEspera" name="tiempoEspera" label="Espera/h" variant="outlined" helperText="tiempo a esperar entre eventos" size="small" type="number" />
          </Grid>
          <Grid item xs={6}>
            <TextField required id="tiempoMaximo" name="tiempoMaximo" label="TiempoMaximo/h" variant="outlined" helperText="tiempo maximo de alquiler" size="small" type="number" />
          </Grid>
        </Grid>
